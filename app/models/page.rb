...
class Page < ActiveRecord::Base
  ...
  def child_path(child)
    ...
    tmp=clean_path(path + '/' + child.slug)

    logger.warn "\n###########################   child_path(#{child})    ####################################"
    logger.warn "#{Time.now.strftime("%H:%M:%S:%3N")} child='#{child[:title]}' '#{child[:slug]}' '#{child[:breadcrumb]}' '#{child[:class_name]}' '#{child[:status_id]}' '#{child[:parent_id]}' childPath=#{tmp.inspect}"
    calls=[]
  ...
  def part(name)
    if new_record? or parts.to_a.any?(&:new_record?)
      ...
      #parts.find(:all,:conditions=>['name =?',name.to_s])
    end
  end
  ...
  def field(name)
    if new_record? or fields.any?(&:new_record?)
      ...
      #fields.find(:all,:conditions=>['name =?',name.to_s])
    end
  end
  ...
  def path
    logger.warn "\n###########################  path()  ####################################"
    if parent?
      pp=parent.child_path(self)
      logger.warn "#{Time.now.strftime("%H:%M:%S:%3N")} parent='#{parent[:title]}' '#{parent[:slug]}' '#{parent[:breadcrumb]}' '#{parent[:class_name]}' '#{parent[:status_id]}' '#{parent[:parent_id]}' Path=#{pp.inspect}"
      calls=[]
      caller.reverse.each{|c| calls<<c.gsub(/www\/radiant\/app\//,'').gsub(/\/(.+)\.rb.*in .(.*)./,'\1::\2') if c=~ /www\/radiant/}
      logger.warn "#{Time.now.strftime("%H:%M:%S:%3N")} caller=#{calls}"
    else
      pp=clean_path(slug)
      logger.warn "#{Time.now.strftime("%H:%M:%S:%3N")} slug=#{slug.inspect} Path=#{pp.inspect}"
      calls=[]
      caller.reverse.each{|c| calls<<c.gsub(/www\/radiant\/app\//,'').gsub(/\/(.+)\.rb.*in .(.*)./,'\1::\2') if c=~ /www\/radiant/}
      logger.warn "#{Time.now.strftime("%H:%M:%S:%3N")} caller=#{calls}"
    end
  ...
  class << self
    ...
    def find_by_pathex(path, live = true)
      logger.warn "###########################   find_by_path_EX(#{path},#{live})    ####################################"
      calls=[]
      caller.reverse.each{|c| calls<<c.gsub(/www\/radiant\/app\//,'').gsub(/\/(.+)\.rb.*in .(.*)./,'\1::\2') if c=~ /www\/radiant/}
      logger.warn "#{Time.now.strftime("%H:%M:%S:%3N")} caller=#{calls}"
      raise MissingRootPageError unless root
      #root.find_by_path(path, live)parent_id is NULL or
      Page.find(:first,:conditions=>['slug = ?',path.gsub(/.*\/(.*)/,'\1')],:include=>:parts)
    end

    def find_by_path(path, live = true)
      logger.warn "###########################   find_by_path(#{path},#{live})    ####################################"
      calls=[]
      caller.reverse.each{|c| calls<<c.gsub(/www\/radiant\/app\//,'').gsub(/\/(.+)\.rb.*in .(.*)./,'\1::\2') if c=~ /www\/radiant/}
      logger.warn "#{Time.now.strftime("%H:%M:%S:%3N")} caller=#{calls}"
      raise MissingRootPageError unless root
  ...
  private
    ...
    def clean_path(path)
      logger.warn "\n###########################   clean_path(#{path})    ####################################"
      cp="/#{ path.to_s.strip }/".gsub(%r{//+}, '/')
#      logger.warn "#{Time.now.strftime("%H:%M:%S:%3N")} caller=#{caller.collect{|c| c.gsub(/.*\/(.+)\.rb.*`(.*)'/,"#{\1.humanize}::#{\2}") if c=~%r{/www/radiant/}}} CleanPth=#{cp.inspect}"
      logger.warn "#{Time.now.strftime("%H:%M:%S:%3N")} path=#{path.inspect} CleanPth=#{cp.inspect}"
      calls=[]
      caller.reverse.each{|c| calls<<c.gsub(/www\/radiant\/app\//,'').gsub(/\/(.+)\.rb.*in .(.*)./,'\1::\2') if c=~ /www\/radiant/}
      logger.warn "#{Time.now.strftime("%H:%M:%S:%3N")} caller=#{calls}"
      cp
