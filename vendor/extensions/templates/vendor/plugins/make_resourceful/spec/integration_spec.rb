...
describe "ThingsController", "with several parent objects", :type => :integration do
  ...
  it "should assign the proper parent variables and accessors to the categories with id 4 for GET /people/4/things" do
  ...
  it "should find all the things belonging to the categories with id 4 on GET /people/4/things" do
  ...
  it "should find the thing with id 12 if it belongs to the categories with id 4 on GET /categories/4/things/12" do
    Category.expects(:find).with('4').returns(@category)
    @category.expects(:things).at_least_once.returns(@fake_model)
    @fake_model.expects(:find).with('12').returns(@object)
  ...
  it "should create a new thing belonging to the categories with id 4 on POST /categories/4/things" do
    Category.expects(:find).with('4').returns(@category)
    @category.expects(:things).at_least_once.returns(@fake_model)
    @fake_model.expects(:build).with('name' => 'Lamp').returns(@object)
