  ...
  module Helpers
    ...
    module ListColumnHelpers
      def get_column_value(record, column)
        logger.warn "#{Time.now.strftime("%H:%M:%S:%3N")} get_column_value record=#{record.inspect} ##### column=#{column.inspect} ###### caller=#{caller}"
      ...
      def cache_association(value, column)
        ...
        unless value.loaded?
          # load at least one record, is needed for column_empty? and checking data_permissions
          if column.associated_limit.nil?
