# This module attempts to create data_permissions conventions for your ActiveRecord models. It supports english-based
...
module ActiveRecordPermissions
  ...
  # This is a module aimed at making the current_user available to ActiveRecord models for data_permissions.
  ...
  module Permissions
    ...
    #
    # options[:action] should be a CRUD verb (:create, :read, :update, :delete)
    # options[:column] should be the name of a model attribute
    def authorized_for?(options = {})
      raise ArgumentError, "unknown action #{options[:action]}" if options[:action] and ![:create, :read, :update, :delete].include?(options[:action])
