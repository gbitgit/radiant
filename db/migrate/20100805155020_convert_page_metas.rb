class ConvertPageMetas < ActiveRecord::Migration
  ...
  def self.down
    ...
    Page.all.each do |page|
      page.description = page.field('description').content unless page.field('description').nil?
      page.keywords = page.field('keywords').content unless page.field('keywords').nil?
      page.save  unless (page.field('keywords').nil? and page.field('description').nil?)
    end
  end
end
