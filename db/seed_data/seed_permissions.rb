...
controllers_and_actions.each do |controller_actions|
				attributes=
				{
				  :code =>(controller_actions[:controller].to_s+'_NODE').upcase,
				  :desc =>(controller_actions[:controller].to_s+'_node'),
...
orgs.each do |org|
				...
				attributes=
				{
				  ...
				  :desc =>(ctrl_filter[:controller]+(ctrl_filter[:controller]==ctrl_filter[:method] ? ' NODE' : ('::'+ctrl_filter[:method]))).upcase,
